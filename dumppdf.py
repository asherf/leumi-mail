...
def extract_attachments(fname, password, extractdir):
    extractembedded("", fname, 0, 0, password=password, extractdir=extractdir, auto_rename=True)

def extractembedded(outfp, fname, objids, pagenos, password='',
                    dumpall=False, codec=None, extractdir=None, auto_rename=False):
    def extract1(obj):
        try:
            filename = unicode(os.path.basename(obj.get('UF') or obj['F']))
        ...
        if not auto_rename:
            ...
            while os.path.exists(path):
                ...
                filename = "%s_%s%s" % (filename, i, file_extension)
                path = os.path.join(extractdir, filename)
